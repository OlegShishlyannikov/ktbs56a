...
static const std::map< int, int > leds_pin_map = {{ 1, LED1_GPIO_PIN }, { 2, LED2_GPIO_PIN }, { 3, LED3_GPIO_PIN }, { 4, LED4_GPIO_PIN }};
static int report = 0;
...
leds_driver::leds_driver( leds_driver & driver )
{
  *this = driver;
}
...
void leds_driver::write( const char * p_params )
{
  ...
  if( std::sscanf( msg, "led_%u_%s", &led_num, cmd ) == 2 ){
	...
	if( !std::strcmp( cmd,  "set" )){
	  ...
	  report = 0;
	  ( leds_pin_map.find( led_num ) != leds_pin_map.end() ) ? GPIO_SetBits( LEDS_GPIO_PORT , leds_pin_map.at( led_num )) : ( void ) 0;

	} else if( !std::strcmp( cmd,  "reset" )){

	  report = 0;
	  ( leds_pin_map.find( led_num ) != leds_pin_map.end() ) ? GPIO_ResetBits( LEDS_GPIO_PORT , leds_pin_map.at( led_num )) : ( void ) 0;
	  
	} else if( !std::strcmp( cmd,  "read_state" )){

	  int led_state = ( leds_pin_map.find( led_num ) != leds_pin_map.end() ) ? GPIO_ReadInputDataBit( LEDS_GPIO_PORT, leds_pin_map.at( led_num )) : 0;
	  report = led_state;
...
{
  if( xQueueReceive( leds_driver_queue_handle, &from_queue, timeout )) return static_cast< void * >( &from_queue );
  else return nullptr;
}
