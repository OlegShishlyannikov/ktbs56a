...
shell::shell() {
  ...
  xTaskCreate(&this->shell_task_code_wrapper, ((this->get_name()) + "_task").c_str(), configMINIMAL_STACK_SIZE * 4,
              nullptr, configAPP_PRIORITY, &shell_task_handle);
  vTaskStartScheduler();
}
...
void shell::handle_input_char(const char c) {
  ...
  update_cursor_pos = [this, starts_with, ends_with](cursor_pos *_pos) -> bool {
    ...
    while (received_char != '\e')
      received_char = *static_cast<char *>(shell::hardware[STD_IO_DRIVER].read(portTERMINAL_MAX_DELAY));
    ...
    while ((!ctrl_seq_completed) && (!ctrl_seq_error)) {
      ...
      if (p_received_char != nullptr) {
        ...
        esc_seq_str += received_char;
        ctrl_seq_completed =
            ((starts_with(esc_seq_str, ASCII_CONTROL_REPORT_CURSOR_POS.substr(0, 2))) &&
             (ends_with(esc_seq_str,
                        ASCII_CONTROL_REPORT_CURSOR_POS.substr(ASCII_CONTROL_REPORT_CURSOR_POS.length() - 1))));
  ...
  split_string = [](std::string received_char) -> std::vector<std::string> {
    ...
    for (std::string str; queue_stream >> str;)
      if (args_vector.size() <= MAX_ARGS)
        args_vector.push_back(str);
  ...
  handle_if_esc_seq = [this, update_cursor_pos](const char c) -> void {
    if (c == ASCII_CTRL_CH_ESC.at(0)) {
      ...
      if ((ctrl_seq_completed) && (!ctrl_seq_error))
        ctrl_seq_responses.at(esc_seq_str)(&args);
    }
  };
  ...
  handle_if_generic_ascii = [this, update_cursor_pos](const char c, std::string *padding) -> void {
    if (((!std::iscntrl(c)) && !(c == ASCII_CTRL_CH_ESC.at(0)) &&
         ((std::isalnum(c) || std::ispunct(c) || std::isspace(c)))) &&
        (input_buffer_stream.length() < MAX_BUFFER_LENGTH)) {
      ...
      try {
        ...
        shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_ERASE_END_OF_LINE + c + ASCII_CONTROL_SAVE_CURSOR +
                                             input_buffer_stream.substr(input_iter + 1) + ASCII_CONTROL_UNSAVE_CURSOR);
  ...
  ctrl_seq_responses = {
      {ASCII_CONTROL_CURSOR_UP,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         ...
         if (history.size()) {
           ...
           if (history_iter >= 0) {
             ...
             shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS + ASCII_CONTROL_ERASE_END_OF_LINE, pos.y,
                                                  padding_length + 1);
         ...
         if (history.size()) {
           ...
           if (history_iter < history.size()) {
             ...
             if (history_iter < history.size()) {
               ...
               input_buffer_stream = history[history_iter];
               shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS + ASCII_CONTROL_ERASE_END_OF_LINE,
                                                    pos.y, padding_length + 1);
               shell::hardware[STD_IO_DRIVER].write(input_buffer_stream);
      ...
      {ASCII_CONTROL_CURSOR_FORWARD,
       [this, update_cursor_pos](void *args) {
         ...
         if (input_iter <= input_buffer_length)
           shell::hardware[STD_IO_DRIVER].write("%s", ASCII_CONTROL_CURSOR_FORWARD.c_str());
       }},

      {ASCII_CONTROL_CURSOR_BACKWARD,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
         ...
         if (input_iter > 1)
           shell::hardware[STD_IO_DRIVER].write("%s", ASCII_CONTROL_CURSOR_BACKWARD.c_str());
       }},

      {ASCII_CONTROL_KBD_CURSOR_CTRL_UP,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
      ...
      {ASCII_CONTROL_KBD_CURSOR_CTRL_DOWN,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
      ...
      {ASCII_CONTROL_KBD_CURSOR_CTRL_FORWARD,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
         ...
         for (int i = input_iter; i < input_buffer_stream.length(); i++)
           ...
           if (std::isspace(input_buffer_stream[i]) || std::ispunct(input_buffer_stream[i])) {

             punct_space_pos = i + padding_length + 1;
             break;
           }

         if (punct_space_pos != -1)
           shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS, pos.y, punct_space_pos);
         else
           shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS, pos.y,
                                                padding_length + input_buffer_length + 1);
       }},

      {ASCII_CONTROL_KBD_CURSOR_CTRL_BACKWARD,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
         ...
         if (punct_space_pos != -1)
           ...
           shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS, pos.y, padding_length + 1);
       }},

      {ASCII_CONTROL_CURSOR_LINE_BEGIN,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
       ...
       [this, update_cursor_pos](void *args) {
         ...
         try {
           ...
           if (input_iter < input_buffer_length) {
             ...
             input_buffer_stream_cache = input_buffer_stream;
             shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SAVE_CURSOR + ASCII_CONTROL_ERASE_END_OF_LINE +
                                                  input_buffer_stream.substr(input_iter) + ASCII_CONTROL_UNSAVE_CURSOR);
           }
         } catch (const std::out_of_range &e) {
      ...
      {ASCII_CONTROL_CURSOR_LINE_END,
       [this, update_cursor_pos](void *args) {
         ...
         shell::hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_CURSOR_POS.c_str(), pos.y,
                                              (padding_length + 1) + input_buffer_length);
       }},

      {ASCII_CONTROL_CURSOR_PAGE_UP,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
      ...
      {ASCII_CONTROL_CURSOR_PAGE_DOWN,
       [this, update_cursor_pos](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
         ...
         try {
           ...
           if (input_iter > 0) {
             ...
             shell::hardware[STD_IO_DRIVER].write(
                 ASCII_CTRL_CH_DEL + ASCII_CONTROL_SAVE_CURSOR + ASCII_CONTROL_ERASE_END_OF_LINE +
                 input_buffer_stream.substr(input_iter - 1) + ASCII_CONTROL_UNSAVE_CURSOR);
           }
         } catch (const std::out_of_range &e) {
      ...
      {ASCII_CTRL_CH_TAB,
       [this, update_cursor_pos, starts_with](void *args) {
         std::tuple<std::string *, std::string *> *args_container =
             static_cast<std::tuple<std::string *, std::string *> *>(args);
         std::string *p_input_buffer = std::get<0>(*args_container);
         std::string *p_padding = std::get<1>(*args_container);
         ...
         for (unsigned int i = 0; i < app_names.size(); i++)
           if (starts_with(app_names[i], input_buffer_stream)) {
             matches++;
             last_match = app_names[i];
           }
         ...
         if (matches) {
           ...
           if (matches == 1) {
             ...
             for (unsigned int i = 0; i < app_names.size(); i++)
               if (starts_with(app_names[i], input_buffer_stream))
                 shell::hardware[STD_IO_DRIVER].write(app_names[i] + "\t");

             hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_ATTRS, ASCII_TERM_ATTRS_BRIGHT, ASCII_TERM_ATTRS_BLACK_BG,
                                           ASCII_TERM_ATTRS_GREEN_FG);
             hardware[STD_IO_DRIVER].write("\r\n" + padding);
             hardware[STD_IO_DRIVER].write(ASCII_CONTROL_SET_ATTRS, ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS,
                                           ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS, ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS);
      ...
      {ASCII_CTRL_CH_CR, [this, update_cursor_pos, split_string](void *args) {
         ...
         if (argv.size()) {
           ...
           if (history.size() < MAX_HISTORY_LENGTH) {
             ...
             else if (input_buffer_stream != history.at(history.size() - 1))
               history.push_back(input_buffer_stream);
           ...
           if (exec(argv[0], &app_args))
             shell::callback_signals.disconnect_all();
...
void shell::shell_task_code(void *parameters) {
  ...
  while (true)
    this->handle_input_char(*static_cast<char *>(hardware[STD_IO_DRIVER].read(portMAX_DELAY)));
