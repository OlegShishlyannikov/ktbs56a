...
#define REGISTER_APPLICATION( x ) TaskHandle_t x##_##task_handle;	\
...
std::vector< std::string > * get_startup_applications()
{
  return &startup_applications;
}
...
bool exec( std::string task_name, std::tuple< std::vector< std::string > *, SemaphoreHandle_t *, shell * > * p_args )
{
  ...
  if( index != -1 ){
	...
	if( task_name.length() > 0 ) shell::hardware[ STD_IO_DRIVER ].write( "\r\nUnknown command \"%s\".\r\n", task_name.c_str() );
