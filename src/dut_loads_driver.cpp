...
dut_loads_driver::dut_loads_driver(dut_loads_driver &driver) { *this = driver; }
...
void *dut_loads_driver::read(TickType_t timeout) const {
  if (xQueueReceive(dut_loads_driver_queue_handle, &from_queue, timeout))
...
int dut_loads_driver::set_current(double current_ma) {
  if (current_ma == 0.0f)
    ...
    DAC_SetChannel1Data(
        DAC_Align_12b_R,
        static_cast<uint16_t>(((current_ma * this->ref_ohms) / (this->ref_voltage * 1000.0f)) * std::pow(2, 12)));
  }
  return 0;
}
