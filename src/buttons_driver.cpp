...
static const std::map<std::string, IRQn_Type> buttons_irq_map = {
    {"minus", BUTTON_MINUS_IRQ_CHANNEL}, {"plus", BUTTON_MINUS_IRQ_CHANNEL}, {"run", BUTTON_MINUS_IRQ_CHANNEL}};
static const std::map<std::string, int> buttons_pin_map = {
    {"minus", BUTTONS_GPIO_MINUS_PIN}, {"plus", BUTTONS_GPIO_PLUS_PIN}, {"run", BUTTONS_GPIO_RUN_PIN}};
static int report = 0;
...
buttons_driver::buttons_driver(buttons_driver &driver) { *this = driver; }
...
void buttons_driver::write(const char *p_params) {
  ...
  if (std::sscanf(msg, "%s", cmd) == 1) {
    ...
    if (!std::strcmp(cmd, "get_run_button_state")) {
      ...
      int button_state = (buttons_pin_map.find("run") != buttons_pin_map.end())
                             ? GPIO_ReadInputDataBit(BUTTONS_GPIO_PORT, buttons_pin_map.at("run"))
                             : 0;
      report = button_state;
      ...
      int button_state = (buttons_pin_map.find("minus") != buttons_pin_map.end())
                             ? GPIO_ReadInputDataBit(BUTTONS_GPIO_PORT, buttons_pin_map.at("minus"))
                             : 0;
      report = button_state;
      xQueueSendToBack(buttons_driver_queue_handle, &report, 0);
      ...
      int button_state = (buttons_pin_map.find("plus") != buttons_pin_map.end())
                             ? GPIO_ReadInputDataBit(BUTTONS_GPIO_PORT, buttons_pin_map.at("plus"))
                             : 0;
      report = button_state;
      xQueueSendToBack(buttons_driver_queue_handle, &report, 0);
      ...
      report = 0;
      (buttons_irq_map.find("minus") != buttons_irq_map.end()) ? NVIC_SetPendingIRQ(buttons_irq_map.at("minus"))
                                                               : (void)0;

    } else if (!std::strcmp(cmd, "emulate_plus_press")) {

      report = 0;
      (buttons_irq_map.find("plus") != buttons_irq_map.end()) ? NVIC_SetPendingIRQ(buttons_irq_map.at("plus"))
                                                              : (void)0;
    }
  }
}
...
void *buttons_driver::read(TickType_t timeout) const {
  if (xQueueReceive(buttons_driver_queue_handle, &from_queue, timeout))
...
void BUTTON_MINUS_IRQHandler() {
  if (EXTI_GetITStatus(BUTTON_MINUS_EXTI_LINE)) {
    ...
    if (buttons_driver_task_irq_woken == pdTRUE)
      taskYIELD();
