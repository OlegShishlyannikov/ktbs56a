...
adc_spi_driver::adc_spi_driver( adc_spi_driver & driver )
{
  *this = driver;
}
...
{
  if( xQueueReceive( adc_spi_driver_queue_handle, &from_queue, timeout )) return static_cast< void * >( &from_queue );
  return nullptr;
}
...
{  	  
  ...
  std::bitset< 16 > data_packet(( adc_spi_driver::ADS1118_ConfRegInitStruct.OS.to_ulong() << ADS1118_SS_POS ) |
  ...
  data_packet = std::bitset< 16 >(( adc_spi_driver::ADS1118_ConfRegInitStruct.OS.to_ulong() << ADS1118_SS_POS ) |
								  ( adc_spi_driver::ADS1118_ConfRegInitStruct.MUX.to_ulong() << ADS1118_MUX_POS ) |
								  ( adc_spi_driver::ADS1118_ConfRegInitStruct.PGA.to_ulong() << ADS1118_PGA_POS ) |
								  ( adc_spi_driver::ADS1118_ConfRegInitStruct.MODE.to_ulong() << ADS1118_OPMODE_POS ) |
								  ( adc_spi_driver::ADS1118_ConfRegInitStruct.DR.to_ulong() << ADS1118_DR_POS ) |
...
int adc_spi_driver::get_temp( unsigned int adc_channel, unsigned int mode )
{
  ...
  (( data << 2 ) >= 0 ) ? measured = static_cast< double >( data / 4 ) * celsius_val : measured = static_cast< double >( data / 4 ) * celsius_val * ( -1 );
  xQueueSendToBack( adc_spi_driver_queue_handle, &measured, 0 );
  return 0;
}
...
int adc_spi_driver::read_calibration_table( unsigned int table_size )
{
  ...
  for( unsigned int i = 0; i < table_size; i ++ ){
	...
	unsigned int lsb = *reinterpret_cast< unsigned int * >( static_cast< size_t >( table_offset + sizeof( unsigned int ) + i * sizeof( double )));
	unsigned long long int dat = static_cast< unsigned long long int >((( std::bitset< 64 >( msb ) << ( sizeof( double ) * 8 / 2 ) ) | std::bitset< 64 >( lsb )).to_ullong());
...
void adc_spi_driver::flash_lock()
{
  FLASH->CR |= FLASH_CR_LOCK;
}

bool adc_spi_driver::flash_ready()
{
...
int adc_spi_driver::flash_write_double( uint32_t address, double data )
{
  ...
  log( "Verify written data (%lf MSB : %u, LSB : %u) on address 0x%x (%lf MSB : %u, LSB : %u) - %s\r\n", data, data_binary_msb, data_binary_lsb, address, readed_double, msb, lsb, ( data == readed_double ) ? "OK" : "FAIL" );
  
  if( readed_double == data ) return 0;
  else return 1;
}
