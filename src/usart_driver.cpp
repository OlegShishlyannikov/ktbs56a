...
usart_driver::usart_driver(usart_driver &driver) { *this = driver; }
...
void *usart_driver::read(TickType_t timeout) const {
  if (xQueueReceive(usart_driver_queue_handle, &rcvd_char, timeout))
    return static_cast<void *>(&rcvd_char);
  else
    return nullptr;
}
...
extern "C" void USART1_IRQHandler() {
  if (USART_GetITStatus(USART_PERIPH_PORT, USART_IT_RXNE)) {
    ...
    if (usart_driver_task_irq_woken == pdTRUE)
      taskYIELD();

  } else
    USART_ClearITPendingBit(USART_PERIPH_PORT, USART_IT_RXNE | USART_IT_CTS | USART_IT_ERR | USART_IT_FE |
                                                   USART_IT_IDLE | USART_IT_LBD | USART_IT_NE | USART_IT_ORE |
                                                   USART_IT_PE | USART_IT_TC | USART_IT_TXE);
}
