...
events_worker::events_worker()
{
  ...
  xTaskCreate( events_worker_task_code_wrapper, events_worker::get_name().c_str(), configMINIMAL_STACK_SIZE / 2, nullptr, configAPP_PRIORITY, &events_worker_task_handle );
}

std::map< int, signal< void * > * > * events_worker::get_event_signals_map()
{
  return &event_signals_map;
}
...
void events_worker::events_worker_task_code( void * parameters )
{
  ...
  while( true ){
	...
	if( event_signals_map.find( event_id ) != event_signals_map.end() ) event_signals_map.at( event_id )->emit( nullptr ); 
