...
void testing( std::tuple< std::vector< std::string > *, SemaphoreHandle_t *, shell * > * p_args )
{
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ USART_CHAR_RECEIVED_EVENT_ID ])->connect([ &rcvd_char ]( void * args = nullptr ) -> void {
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ TIMER_0_EVENT_ID ])->connect([]( void * args = nullptr ) -> void {
	  ...
	  if( temp_status == TEMP_STATUS_FAULT ){
		...
		for( unsigned int i = 0; i < 2; i ++ ){
		  ...
		  shell::hardware[ "fpga_driver" ].write( "%u_red_led_on", (( dut_number % 2 ) == 1 ) ? dut_number - 1 : dut_number + 1 );	  
		  vTaskDelay( 25 );
		  shell::hardware[ "fpga_driver" ].write( "%u_red_led_off", dut_number );	  
		  shell::hardware[ "fpga_driver" ].write( "%u_red_led_off", (( dut_number % 2 ) == 1 ) ? dut_number - 1 : dut_number + 1 );	  
		  vTaskDelay( 25 );
		  shell::hardware[ "fpga_driver" ].write( "%u_red_led_on", dut_number );	  
		  shell::hardware[ "fpga_driver" ].write( "%u_red_led_on", (( dut_number % 2 ) == 1 ) ? dut_number - 1 : dut_number + 1 );	  
		  vTaskDelay( 25 );
  ...
  cell_overheat.connect([]( int cell_number, double temp ) -> void {
	  ...
	  shell::hardware[ STD_IO_DRIVER ].write( ASCII_CONTROL_SET_ATTRS + "[!] Overheat on cell #%u (%lf C%s). Task stopped.\r\n" + ASCII_CONTROL_SET_ATTRS, ASCII_TERM_ATTRS_BRIGHT, ASCII_TERM_ATTRS_BLACK_BG, ASCII_TERM_ATTRS_RED_FG,
											 dut_number / 2, adc_spi_temp, ASCII_EXT_CH_MASCILINE_IND.c_str(),
											 ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS, ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS, ASCII_TERM_ATTRS_CLEAR_ALL_ATTRS );
	});
  
  if( std::get< 0 >( *p_args )->size() > 0 ) shell::hardware[ STD_IO_DRIVER ].write( "\r\n" );
	
  for( unsigned int i = 0; i < std::get< 0 >( *p_args )->size(); i ++ ) shell::hardware[ STD_IO_DRIVER ].write( "arg#%u : %s\r\n", i, std::get< 0 >( *p_args )->at( i ).c_str() );
  ...
  if( *static_cast< int * >( shell::hardware[ "fpga_driver" ].read( portIO_MAX_DELAY ))){
	...
	( *static_cast< int * >( shell::hardware[ "leds_driver" ].read( portIO_MAX_DELAY )) == 0 ) ? shell::hardware[ "leds_driver" ].write( "led_%u_set", 3 ) : ( void ) 0;

	shell::hardware[ "leds_driver" ].write( "led_%u_read_state", 4 );
	( *static_cast< int * >( shell::hardware[ "leds_driver" ].read( portIO_MAX_DELAY )) == 1 ) ? shell::hardware[ "leds_driver" ].write( "led_%u_reset", 4 ) : ( void ) 0;

  } else {

	shell::hardware[ "leds_driver" ].write( "led_%u_read_state", 4 );
	( *static_cast< int * >( shell::hardware[ "leds_driver" ].read( portIO_MAX_DELAY )) == 0 ) ? shell::hardware[ "leds_driver" ].write( "led_%u_set", 4 ) : ( void ) 0;

	shell::hardware[ "leds_driver" ].write( "led_%u_read_state", 3 );
	( *static_cast< int * >( shell::hardware[ "leds_driver" ].read( portIO_MAX_DELAY )) == 1 ) ? shell::hardware[ "leds_driver" ].write( "led_%u_reset", 3 ) : ( void ) 0;
	
  }
  
  shell::hardware[ "adc_spi_driver" ].write( "read_calibration_table_size_%u", DUTS_NUMBER * output_voltages.size() );
  double * p_readed_table = reinterpret_cast< double * >( static_cast< size_t >(( *static_cast< double * >( shell::hardware[ "adc_spi_driver" ].read( portIO_MAX_DELAY )))));
   
  for( unsigned int i = 0; i < DUTS_NUMBER * output_voltages.size(); i ++ ){

	if( static_errors_k.find( "ch_" + std::to_string( i % DUTS_NUMBER ) + "_" + std::to_string( output_voltages[ i / DUTS_NUMBER ]) + "_VREF" ) == static_errors_k.end() )
	  static_errors_k.insert( std::make_pair( "ch_" + std::to_string( i % DUTS_NUMBER ) + "_" + std::to_string( output_voltages[ i / DUTS_NUMBER ]) + "_VREF", p_readed_table[ i ]));

	else static_errors_k.at( "ch_" + std::to_string( i % DUTS_NUMBER ) + "_" + std::to_string( output_voltages[ i / DUTS_NUMBER ]) + "_VREF" ) = p_readed_table[ i ];
  ...
  std::function< double( double )> get_range = []( double data ) -> double {
	...
	for( double output_voltage : output_voltages ){
	  ...
	  if( data >= output_voltage ) min = output_voltage;
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ BUTTON_MINUS_PRESS_EVENT_ID ])->connect([ &rcvd_char ]( void * args = nullptr ) -> void {
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ BUTTON_PLUS_PRESS_EVENT_ID ])->connect([ &rcvd_char ]( void * args = nullptr ) -> void {
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ BUTTON_RUN_PRESS_EVENT_ID ])->connect([ &rcvd_char ]( void * args = nullptr ) -> void {
  ...
  while( true ){
	...
	if( fsm_state == FSM_MEASUREMENT_STATE ){
	  ...
	  if( static_errors_k.find( "ch_" + std::to_string( dut_number ) + "_" + std::to_string( vref_range ) + "_VREF" ) != static_errors_k.end() )
		adc_spi_data += static_errors_k.at( "ch_" + std::to_string( dut_number ) + "_" + std::to_string( vref_range ) + "_VREF" );
	  ...
	  shell::hardware[ STD_IO_DRIVER ].write( "CS_ADC : %i\tCS_DAC : %i, Setting current : %lf, Measured : %lf, Cell_temperature : %lf, DUTn : %u\r\n",
											 dut_loads_adc_val,
											 dut_loads_dac_val,
											 dut_current,
											 adc_spi_data,
	  ...
	  if( adc_spi_temp >= maximum_device_temperature ) temp_status = TEMP_STATUS_FAULT;	
	  else temp_status = TEMP_STATUS_OK; 
	  ...
	  if( temp_status == TEMP_STATUS_FAULT ){
		...
		while( temp_status == TEMP_STATUS_FAULT ) vTaskDelay( 100 );
	  ...
	  if( dut_number == DUTS_NUMBER - 1 ) dut_number = 0;
	  else dut_number ++;
...
void calibrate_adcs( std::tuple< std::vector< std::string > *, SemaphoreHandle_t *, shell * > * p_args )
{
  ...
  static_cast< signal< void * > * >( shell::callback_signals[ USART_CHAR_RECEIVED_EVENT_ID ])->connect([ &received_char ]( void * args = nullptr ) -> void {
  ...
  for( unsigned int k = 0; k < output_voltages.size(); k ++ ){
  	...
  	while( received_char != '\r' ) vTaskDelay( 100 );
  	...
  	for( unsigned int i = 0; i < DUTS_NUMBER; i ++ ){
  	  ...
  	  static_errors_k.insert( std::make_pair( "ch_" + std::to_string( i ) + "_" + std::to_string( output_voltages[ k ]) + "_VREF", calibration_table[ i ]));
  	  shell::hardware[ STD_IO_DRIVER ].write( "." );

  	}	
  }
  
  shell::hardware[ "adc_spi_driver" ].write( "write_calibration_table_%p_%u", calibration_table, DUTS_NUMBER * output_voltages.size() );
  shell::hardware[ "adc_spi_driver" ].write( "read_calibration_table_size_%u", DUTS_NUMBER * output_voltages.size() );
  double * p_readed_table = reinterpret_cast< double * >( static_cast< size_t >(( *static_cast< double * >( shell::hardware[ "adc_spi_driver" ].read( portIO_MAX_DELAY )))));
...
{
  std::string text;
  for( unsigned int i = 1; i < std::get< 0 >( *p_args )->size(); i ++ ) text += std::get< 0 >( *p_args )->at( i ) + " ";

  if( !text.length() ) text = "What?";
