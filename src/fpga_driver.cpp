...
{
  this->init();
  GPIO_SetBits( FPGA_GPIO_PORT, FPGA_GPIO_M0_PIN | FPGA_GPIO_M1_PIN | FPGA_GPIO_VS0_PIN | FPGA_GPIO_VS1_PIN | FPGA_GPIO_VS2_PIN | FPGA_GPIO_NRST_PIN );
...
fpga_driver::fpga_driver( fpga_driver & driver )
{
  *this = driver;
}
...
{
  if( xQueueReceive( fpga_driver_queue_handle, &report, timeout )) return static_cast< void * >( &report );
  else return nullptr;
}
...
int fpga_driver::init()
{
  ...
  GPIO_InitStruct->GPIO_Pin = FPGA_GPIO_VS0_PIN | FPGA_GPIO_VS1_PIN | FPGA_GPIO_VS2_PIN | FPGA_GPIO_M0_PIN | FPGA_GPIO_M1_PIN | FPGA_GPIO_M2_PIN | FPGA_GPIO_NCS_PIN | FPGA_GPIO_NRST_PIN | FPGA_GPIO_EN_PIN;
...
int fpga_driver::green_led_on( unsigned int led_num )
{
  return fpga_driver::send_packet( GREEN_LED_ON_COMMAND, led_num );
}
...
int fpga_driver::red_led_on( unsigned int led_num )
{
  return fpga_driver::send_packet( RED_LED_ON_COMMAND, led_num );
}

int fpga_driver::red_led_off( unsigned int led_num )
{
  return fpga_driver::send_packet( RED_LED_OFF_COMMAND, led_num );
}

int fpga_driver::adc_select( unsigned int adc_num )
{
  return fpga_driver::send_packet( ADC_SELECT_COMMAND, adc_num );
}

int fpga_driver::adc_deselect( unsigned int adc_num )
{
  return fpga_driver::send_packet( ADC_DESELECT_COMMAND, adc_num );
}

int fpga_driver::adc_deselect_all()
{
  return fpga_driver::send_packet( ADC_DESELECT_COMMAND, 0 );
}

int fpga_driver::reset_all()
{
  return fpga_driver::send_packet( RESET_ALL_COMMAND, 0 );
}

int fpga_driver::get_conf_status()
{
  return GPIO_ReadInputDataBit( FPGA_GPIO_PORT, FPGA_GPIO_DONE_PIN );
}
