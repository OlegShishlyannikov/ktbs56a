...
static const std::map< unsigned int, TIM_TypeDef * > tim_periph_map = {{ 0, TIM0_PERIPH }, { 1, TIM1_PERIPH }, { 2, TIM2_PERIPH }, { 3, TIM3_PERIPH }};
static int report = 0;
...
gp_timers_driver::gp_timers_driver( gp_timers_driver & driver )
{
  *this = driver;
}
...
void gp_timers_driver::write( const char * p_params )
{
  ...
  if( std::sscanf( msg, "timer_%u_%s", &tim_number, cmd ) == 2 ){
	...
	if( !std::strcmp( cmd, "start" )){
	  ...
	  report = 0;
	  if( tim_periph_map.find( tim_number ) != tim_periph_map.end() ) TIM_Cmd( tim_periph_map.at( tim_number ), ENABLE );
	  
	} else if( !std::strcmp( cmd, "stop" )){

	  report = 0;
	  if( tim_periph_map.find( tim_number ) != tim_periph_map.end() ) TIM_Cmd( tim_periph_map.at( tim_number ), DISABLE );
	  
	} else if( !std::strcmp( cmd, "reset" )){

	  report = 0;
	  if( tim_periph_map.find( tim_number ) != tim_periph_map.end() ) TIM_SetCounter( tim_periph_map.at( tim_number ), 0x0000 );
...
{
  if( xQueueReceive( gp_timers_driver_queue_handle, &from_queue, timeout )) return static_cast< void * >( &from_queue ); 
  else return nullptr;
}
...
extern "C" {
  ...
  void TIM1_UP_TIM10_IRQHandler()
  {
	if( TIM_GetITStatus( TIM0_PERIPH, TIM_IT_Update )){
	  ...
	  if( gp_timers_driver_task_irq_woken == pdTRUE ) taskYIELD();
	
	} else TIM_ClearITPendingBit( TIM0_PERIPH, TIM_IT_Break | TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_CC3 | TIM_IT_CC4 | TIM_IT_COM | TIM_IT_Trigger | TIM_IT_Update );
  }
  ...
  void TIM2_IRQHandler()
  {
	...
	} else TIM_ClearITPendingBit( TIM1_PERIPH, TIM_IT_Break | TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_CC3 | TIM_IT_CC4 | TIM_IT_COM | TIM_IT_Trigger | TIM_IT_Update );
  }
  ...
  void TIM3_IRQHandler()
  {
	...
	} else TIM_ClearITPendingBit( TIM2_PERIPH, TIM_IT_Break | TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_CC3 | TIM_IT_CC4 | TIM_IT_COM | TIM_IT_Trigger | TIM_IT_Update );
  }
  ...
  void TIM4_IRQHandler()
  {
	...
	} else TIM_ClearITPendingBit( TIM3_PERIPH, TIM_IT_Break | TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_CC3 | TIM_IT_CC4 | TIM_IT_COM | TIM_IT_Trigger | TIM_IT_Update );
  }
}
